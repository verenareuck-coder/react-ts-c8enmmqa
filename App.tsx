...
// - El selector "Trabajador / Gerencia" es solo una demostración visual, no
//   un login real: cualquiera que abra la app puede cambiarlo.

import React, { useState, useEffect, useCallback, useMemo, useRef } from 'react';
import {
  Search, Bell, Package, Users, ShoppingCart, TrendingUp, FileText,
  AlertTriangle, Truck, Wallet, BarChart3, RefreshCw, Wifi, WifiOff,
} from 'lucide-react';
import {
...
// Compras MMPP: además de los campos base, busca columnas "Boleto1 Vencimiento",
...
/* ------------------------------- caché local -------------------------------- */
// Reemplaza al window.storage de Claude: aquí usamos localStorage del
// navegador, disponible en cualquier entorno normal.
...
// Intenta traer datos en vivo desde Google Sheets. Si falla, usa el último
// dato guardado; si tampoco hay, usa datos de ejemplo. Siempre informa la
// fuente para poder mostrar el estado de conexión en la interfaz.
    ...
    <aside style={{ background: C.navy, width: 232, flexShrink: 0 }} className="flex flex-col py-5">
      <div className="px-4 pb-5 mb-2" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        <div className="flex items-center gap-2">
          <div style={{ background: C.salmon, width: 28, height: 28, borderRadius: 6 }} className="flex items-center justify-center text-white font-semibold text-sm">A</div>
          <div>
            <div className="text-white text-sm font-medium leading-tight">Panel de gestión</div>
...
function TopBar({ role, setRole, alerts, notifOpen, setNotifOpen, lastSync, connectionStatus }) {
  ...
  return (
    <header style={{ borderBottom: `1px solid ${C.border}`, background: C.surface }} className="flex items-center justify-between px-6 py-3">
      ...
      <div className="flex items-center gap-3">
        <div
          title="Selector de vista para esta demo — no es un login real"
          style={{ border: `1px solid ${C.border}` }}
          className="flex rounded-md overflow-hidden text-xs"
        >
          <button onClick={() => setRole('trabajador')} style={{ background: role === 'trabajador' ? C.navy : C.surface, color: role === 'trabajador' ? '#fff' : C.textMuted }} className="px-3 py-1.5">Trabajador</button>
          <button onClick={() => setRole('gerencia')} style={{ background: role === 'gerencia' ? C.navy : C.surface, color: role === 'gerencia' ? '#fff' : C.textMuted }} className="px-3 py-1.5">Gerencia</button>
        </div>
...
function VentasView({ ventas }) {
  ...
  const rowStyle = (estado) => {
    ...
    return { background: C.neutralBg, color: C.textMuted }; // Sin definir
  };
  ...
  return (
    <div style={{ background: C.bg, color: C.text, minHeight: '100vh', fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }} className="flex text-sm">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <TopBar role={role} setRole={setRole} alerts={alerts} notifOpen={notifOpen} setNotifOpen={setNotifOpen} lastSync={lastSync} connectionStatus={connectionStatus} />
